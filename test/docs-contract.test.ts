...
test('the README, .env.example and config.ts name the same variables', async () => {
  ...
  const table = readme.slice(readme.indexOf('## Configuration'), readme.indexOf('### When is the weekend?'));
  const documented = names(table);
  ...
  for (const name of read) {
    assert.ok(documented.has(name), `README's configuration table never mentions ${name}`);
